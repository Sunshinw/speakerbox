...
| Line | Name | Topic |
|---|---|---|
| 1–44 | imports | argparse, pandas, HF datasets, local `main.py` import via `sys.path` |
...
| 178 | <b> def eval_model <b> | Evaluates a saved model; returns `(accuracy, precision, recall, loss)` and writes `results.md` |
...
| 347 | <b> def train <b> | Main training entry point — device setup, model init, metadata caching, HF Trainer construction and `.train()` call |
